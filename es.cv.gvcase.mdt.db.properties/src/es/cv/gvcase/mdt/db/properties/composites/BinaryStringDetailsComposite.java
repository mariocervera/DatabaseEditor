...
/**
 * The composite that hosts the widgets to manage the details of the BinaryStringDataType
 * 
 * @author mcervera
 */
public class BinaryStringDetailsComposite extends DetailComposite {
	 ...
	 * @author mcervera
	 */
	public BinaryStringDetailsComposite(Composite parent, int style) {
		super(parent, style);
	}
	...
	public void createWidgets(Composite composite, TabbedPropertySheetWidgetFactory widgetFactory) {
		...
		label = widgetFactory.createCLabel(this, "Length:");
	...
	private EStructuralFeature getFeature() {
		return SQLDataTypesPackage.eINSTANCE.getBinaryStringDataType_Length();
	}
	
	
	/**
	 * Gets the property length of the selected BinaryStringDataType
	 * 
	...
	private int getElementLength() {
		if(getElement() != null && getElement() instanceof BinaryStringDataType) {
			return ((BinaryStringDataType)getElement()).getLength();
		}
